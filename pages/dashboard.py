...
def show_recent_transactions():
    ...
    # Pobierz ostatnie transakcje akcji
    try:
        query = """
            SELECT 
                st.transaction_date as data,
                'Akcje' as typ,
                s.symbol,
                st.transaction_type as operacja,
                st.quantity as ilosc,
                st.price_usd as cena,
                (st.quantity * st.price_usd) as wartosc
